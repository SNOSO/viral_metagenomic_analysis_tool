...
if __name__ == "__main__":
    # Interactive user inputs with whitespace stripping
    fastq_file = input("Enter the path to the FASTQ file: ").strip()
    fasta_file = input("Enter the path to the FASTA file of the reference genome sequences: ").strip()
    quality_threshold = int(input("Enter the quality threshold for filtering sequences: ").strip())
    kmer_size = int(input("Enter the size of the k-mers to create: ").strip())
    output_dir = input("Enter the directory to save the summary files: ").strip()
